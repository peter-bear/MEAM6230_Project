...
class Params:
    ...
    selected_primitives_ids: str = '9'  # id number from dataset_keys.py, e.g., '2' or '4,0,6'
    ...
    use_residual: bool = True  # true to add residual connections in encoder and decoder
    ...
    def __init__(self, results_base_directory):
        ...
        if self.use_cosine_annealing:
            suffixes.append('cosannealing')
        if self.use_residual:
            suffixes.append('residual')
        if self.use_distance_aware_alpha:
            suffixes.append('distalpha')
        suffix = ('_' + '_'.join(suffixes)) if suffixes else ''
        self.results_path = results_base_directory + 'results/1st_order_2D' + suffix + '/'
