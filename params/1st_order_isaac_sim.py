...
class Params:
    """ General parameters """
    dataset_name: str = 'isaac_sim_learned'  # options: LASA, LAIR, optitrack, interpolation, joint_space, isaac_sim_learned
    results_path: str = 'results/1st_order_isaac_sim/'
    multi_motion: bool = False  # true when learning multiple motions together
    selected_primitives_ids: str = '2'  # id 2 = pick_and_placed (from dataset_keys.py)
    workspace_dimensions: int = 7  # dimensionality of the data (7 joints from Isaac Sim: J0-J6)
    ...
    use_cosine_annealing: bool = False  # true to use cosine annealing lr scheduler
    use_residual: bool = False  # true to add residual connections in encoder and decoder
    use_distance_aware_alpha: bool = False  # true to use distance-to-goal as extra input to gain network

    """ Preprocessing """
    spline_sample_type: str = 'from data'  # resample from spline type, options: from data, from data resample, evenly spaced
    workspace_boundaries_type: str = 'from data'  # use 'from data' to auto-compute from trajectory data
    workspace_boundaries: np.ndarray = np.array([[-2.0, 2.0]] * 7)  # placeholder, will be overridden by 'from data'
