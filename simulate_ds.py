...
# Initialize dynamical system (device-aware)
device = getattr(learner, 'device', torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
initial_states = torch.tensor(x_t_init, dtype=torch.float, device=device)
dynamical_system = learner.init_dynamical_system(initial_states=initial_states)
