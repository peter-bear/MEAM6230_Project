...
class ContrastiveImitation:
    ...
    def __init__(self, data, params):
        ...
        # Dynamical-system-only params will be initialized after tensors are moved to device
        ...
        self.model = NeuralNetwork(
            ...
            use_distance_aware_alpha=params.use_distance_aware_alpha,
        ).to(device)
        self.device = device

        # Convert data arrays to device tensors
        self.goals_tensor = torch.tensor(data['goals training'], dtype=torch.float, device=self.device)
        self.min_vel = torch.from_numpy(data['vel min train'].reshape([1, self.dim_workspace])).float().to(self.device)
        self.max_vel = torch.from_numpy(data['vel max train'].reshape([1, self.dim_workspace])).float().to(self.device)
        if data['acc min train'] is not None:
            min_acc = torch.from_numpy(data['acc min train'].reshape([1, self.dim_workspace])).float().to(self.device)
            max_acc = torch.from_numpy(data['acc max train'].reshape([1, self.dim_workspace])).float().to(self.device)
        else:
            min_acc = None
            max_acc = None

        # Dynamical-system-only params
        self.params_dynamical_system = {
            'saturate transition': params.saturate_out_of_boundaries_transitions,
            'x min': data['x min'],
            'x max': data['x max'],
        ...
        self.use_cosine_annealing = params.use_cosine_annealing
        ...
        if self.use_cosine_annealing:
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer,
                T_max=params.max_iterations,
                eta_min=1e-6
            )

        # Load Neural Network if requested (map to device)
        if self.load_model:
            self.model.load_state_dict(torch.load(self.results_path + 'model', map_location=self.device), strict=False)
    ...
    def init_dynamical_system(self, initial_states, primitive_type=None, delta_t=1):
        ...
        if primitive_type is None:
            primitive_type = torch.tensor([1.0], dtype=torch.float, device=self.device)
    ...
    def imitation_cost(self, state_sample, primitive_type_sample):
        ...
        for i in range(self.imitation_window_size - 1):
            ...
            imitation_error_accumulated += self.mse_loss(
                x_t_d[:, : self.dim_workspace], state_sample[:, : self.dim_workspace, i + 1].to(self.device)
            )
    ...
    def contrastive_matching(self, state_sample, primitive_type_sample):
        ...
        gap = None  # gap between task and latent trajectories, fed back to gain network
        ...
        for i in range(self.generalization_window_size):
            ...
            _, y_t_latent = dynamical_system_latent.transition_latent_system(gap=gap)

            # Compute gap ||y_t_task - y_t_latent|| for the next step's gain network
            gap = torch.norm(y_t_task.detach() - y_t_latent.detach(), dim=1, keepdim=True)
            ...
            if i > 0:  # we need at least one iteration to have a previous point to push the current one away from
                ...
                if self.stabilization_loss == 'contrastive':
                    ...
                    # Contrastive label
                    contrastive_label_pos = torch.ones(batch_size, device=self.device)
                    contrastive_label_neg = torch.zeros(batch_size, device=self.device)
    ...
    def demo_sample(self):
        ...
        position_sample = torch.tensor(position_sample, dtype=torch.float, device=self.device)

        # Create empty state
        state_sample = torch.empty([self.batch_size, self.dim_state, self.imitation_window_size], device=self.device)
        ...
        primitive_type_sample = torch.tensor(self.primitive_ids[selected_demos], dtype=torch.float, device=self.device)
    ...
    def space_sample(self):
        ...
        从归一化状态空间[-1, 1]^n里面均匀随机采样一批状态点，不是示范轨迹上的点。
        """
        with torch.no_grad():
            # Sample state
            state_sample_gen = torch.empty(self.batch_size, self.dim_state, device=self.device).uniform_(-1, 1)

            # Choose sampling methods
            if not self.multi_motion:
                primitive_type_sample_gen = torch.randint(0, self.n_primitives, (self.batch_size,), device=self.device)
            else:
                # If multi-motion learning also sample in interpolation space
                encodings = torch.eye(self.n_primitives, device=self.device)
                primitive_type_sample_gen_demo = encodings[
                    torch.randint(0, self.n_primitives, (round(self.batch_size * self.interpolation_sigma),), device=self.device)
                ]

                primitive_type_sample_gen_inter = torch.rand(
                    round(self.batch_size * (1 - self.interpolation_sigma)), self.n_primitives, device=self.device
                )
